...
def transfer(block, pretrained):
   ...
   return base_vgg
 #regression model
def fcnet(n_size, fc_shapes, n_dimention):
   layers = []
   in_shape = n_size
   for out_shape in fc_shapes:
       ...
       in_shape = out_shape
   regression = torch.nn.Linear(in_features=in_shape, out_features=n_dimention)
   return torch.nn.ModuleList(layers), regression

class Network(torch.nn.Module):

   def __init__(self, block, input_shape, fc_shapes=[], n_dimention=1, pretrained=True):
